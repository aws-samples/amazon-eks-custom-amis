...
variable "hardening_flag" {
  ...
  validation {
    ...
    error_message = "RHEL8: Only cis, cui, e8, hipaa, ospp, pci-dss, and stig are supported for the hardening flag."
  }
}
...
variable "source_ami_name" {
  type    = string
  default = "RHEL-8*_HVM-*-x86_64-0-Hourly2-GP2"
}
...
locals {
  ami_description = "EKS Kubernetes Worker AMI on RHEL 8 (k8s: ${var.eks_version})"
  target_ami_name = "amazon-eks-node-${var.eks_version}-rhel8-${local.timestamp}"
}
...
build {
  ...
  provisioner "shell" {
    ...
    pause_after       = "30s"
    scripts           = ["./scripts/rhel8/boilerplate.sh"]
  }
  ...
  provisioner "shell" {
    environment_vars = ["HTTP_PROXY=${var.http_proxy}", "HTTPS_PROXY=${var.https_proxy}", "NO_PROXY=${var.no_proxy}", "KUBERNETES_VERSION=${var.eks_version}", "KUBERNETES_BUILD_DATE=${var.eks_build_date}", "BINARY_BUCKET_NAME=${var.binary_bucket_name}", "BINARY_BUCKET_REGION=${var.binary_bucket_region}", "CNI_PLUGIN_VERSION=${var.cni_plugin_version}", "HARDENING_FLAG=${var.hardening_flag}"]
    execute_command  = "echo 'packer' | {{ .Vars }} sudo -S -E bash -eux '{{ .Path }}'"
    scripts          = ["./scripts/rhel8/hardening.sh", "./scripts/rhel8/cleanup.sh"]
  }
